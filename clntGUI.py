...
class GUI:
    ...
    def __init__(self, ip_address, port):
        ...
        self.serveripLabel = tk.Label(self.login, text="Server IP: ", font="Helvetica 12")
        self.serveripLabel.place(relheight=0.2, relx=0.1, rely=0.55)

        self.serveripEntry = tk.Entry(self.login, font="Helvetica 11")
        self.serveripEntry.place(relwidth=0.4 ,relheight=0.1, relx=0.35, rely=0.60)
        
        self.go = tk.Button(self.login, 
                            text="CONTINUE", 
                            font="Helvetica 12 bold", 
                            command = lambda: self.goAhead(self.userEntryName.get(), self.serveripEntry.get(), port, self.roomEntryName.get()))
    ...
    def goAhead(self, username, servip, port, room_id=0):
        self.server.connect((servip, port))
        self.name = username
